  ...
  if (event.target.innerHTML) {
    ...
    if(dropzone.getAttribute('class') == 'drag_here'){
      ...
      infoBut2.forEach(btn => {
        if(btn.parentNode.parentNode.parentNode.getAttribute('class') != "dragging_elements"){
...
function createRow() {
  ...
  for(let l = 0; l < 3; l++){
    ...
    `<div class="hidden_info_div" id="${aaa}">
      <div class="conts">
...
function addInfo(){
  ...
  if(div[id].getAttribute('id') == id){
    ...
    sub[id].setAttribute('href', this.parentNode.parentNode.getAttribute('href'));
